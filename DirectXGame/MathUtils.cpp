...
Matrix4x4 Inverse(const Matrix4x4& matrix) {
	...
	for (int row = 0; row < 4; ++row) {
		for (int col = 0; col < 4; ++col) {
			...
			// 小行列
			float det3 = Determinant3x3(sub[0][0], sub[0][1], sub[0][2], sub[1][0], sub[1][1], sub[1][2], sub[2][0], sub[2][1], sub[2][2]);
...
bool IsCollision(const Sphere& sphere, const Plane& plane) {
	float d = plane.normal.x * sphere.center.x + plane.normal.y * sphere.center.y + plane.normal.z * sphere.center.z - plane.distance;
...
// サイン波アニメーション
inline float SineWave(float time, float speed, float amplitude) { return std::sin(time * speed) * amplitude; }
